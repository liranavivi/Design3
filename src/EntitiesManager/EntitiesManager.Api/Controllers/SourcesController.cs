...
public class SourcesController : ControllerBase
{
    ...
    public async Task<ActionResult<SourceEntity>> GetByCompositeKey(string address, string version)
    {
        ...
        _logger.LogInformation("Starting GetByCompositeKey source request. Address: {Address}, Version: {Version}, CompositeKey: {CompositeKey}, User: {User}, RequestId: {RequestId}",
        ...
        try
        {
            ...
            if (entity == null)
            {
                _logger.LogWarning("Source entity not found by composite key. Address: {Address}, Version: {Version}, CompositeKey: {CompositeKey}, User: {User}, RequestId: {RequestId}",
                    address, version, compositeKey, userContext, HttpContext.TraceIdentifier);
        ...
        {
            _logger.LogError(ex, "Error retrieving source entity by composite key. Address: {Address}, Version: {Version}, CompositeKey: {CompositeKey}, User: {User}, RequestId: {RequestId}",
                address, version, compositeKey, userContext, HttpContext.TraceIdentifier);
            return StatusCode(500, "An error occurred while retrieving the source entity");
        }
    }

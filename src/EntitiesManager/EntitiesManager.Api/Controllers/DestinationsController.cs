...
public class DestinationsController : ControllerBase
{
    ...
    public async Task<ActionResult<DestinationEntity>> GetByCompositeKey(string address, string version)
    {
        var userContext = User.Identity?.Name ?? "Anonymous";
        var compositeKey = $"{address}_{version}";

        _logger.LogInformation("Starting GetByCompositeKey destination request. Address: {Address}, Version: {Version}, CompositeKey: {CompositeKey}, User: {User}, RequestId: {RequestId}",
        ...
        try
        {
            ...
            if (entity == null)
            {
                _logger.LogWarning("Destination entity not found by composite key. Address: {Address}, Version: {Version}, CompositeKey: {CompositeKey}, User: {User}, RequestId: {RequestId}",
                    address, version, compositeKey, userContext, HttpContext.TraceIdentifier);
                return NotFound($"Destination with address '{address}' and version '{version}' not found");
            }
        ...
        {
            _logger.LogError(ex, "Error retrieving destination entity by composite key. Address: {Address}, Version: {Version}, CompositeKey: {CompositeKey}, User: {User}, RequestId: {RequestId}",
                address, version, compositeKey, userContext, HttpContext.TraceIdentifier);
            return StatusCode(500, "An error occurred while retrieving the destination entity");
        }
    }

...
public class UpdateImporterCommandConsumer : IConsumer<UpdateImporterCommand>
{
    private readonly IImporterEntityRepository _repository;
    private readonly IPublishEndpoint _publishEndpoint;
    private readonly ILogger<UpdateImporterCommandConsumer> _logger;

    public UpdateImporterCommandConsumer(
        IImporterEntityRepository repository,
        IPublishEndpoint publishEndpoint,
        ILogger<UpdateImporterCommandConsumer> logger)
    {
    ...
    public async Task Consume(ConsumeContext<UpdateImporterCommand> context)
    {
        _logger.LogInformation("Processing UpdateImporterCommand for ID {Id}", context.Message.Id);
        ...
        try
        {
            ...
            if (existing == null)
            {
                _logger.LogWarning("ImporterEntity with ID {Id} not found for update", context.Message.Id);
            ...
            _logger.LogInformation("Successfully processed UpdateImporterCommand for ID {Id}", context.Message.Id);
        }
        catch (DuplicateKeyException ex)
        {
            _logger.LogWarning("Duplicate key error in UpdateImporterCommand: {Error}", ex.Message);
            await context.RespondAsync(new { Error = ex.Message, Success = false });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing UpdateImporterCommand for ID {Id}", context.Message.Id);
            throw;
        }
    }
}
